...
export class ReviewsService {
  ...
  async createReview(
    ...
    if (!createReviewDto) {
      throw new BadRequestException('Enter reviews date stars or comment');
    }
    try {
      ...
      throw new InternalServerErrorException(
        'Failed to create review due to an unexpected server error.'
      );
    }
  }
  ...
  async deleteReviewComment(reviewId: string, userId: string): Promise<Review> {
    try {
      ...
      return await this.reviewModel
        .findByIdAndUpdate(reviewId, { $unset: { comments: 1 } }, { new: true })
        .exec();
    } catch (error) {
      ...
      throw new InternalServerErrorException(
        'Failed to delete review due to an unexpected server error.'
      );
    }
  }
  ...
  async updateReview(
    ...
    try {
      ...
      const updatedReview = await this.reviewModel
        .findByIdAndUpdate(reviewId, updateDto, { new: true })
        .exec();
      if (!updatedReview) {
        throw new NotFoundException('Review not found');
      }
      ...
      throw new InternalServerErrorException(
        'Failed to update review due to an unexpected server error.'
      );
    }
  }
  ...
  async getAllReviewsForProduct(
    ...
    try {
      ...
      const response = {
        ...
        summary: {
          averageStarsRating: parseFloat(
            (summary.averageStarsRating || 0).toFixed(1)
          ),
