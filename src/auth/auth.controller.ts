...
export class AuthController {
  ...
  @ApiResponse({
    status: 302,
    description: 'Redirects to home page with auth cookie'
  })
  ...
  async googleAuthRedirect(
    ...
    try {
      ...
      if (userType) {
        if (userType === UserRole.NORMAL_USER || userType === UserRole.SELLER) {
          ...
          throw new BadRequestException(
            'Invalid user type. Must be "normal_user" or "seller".'
          );
        }
      }
      ...
      throw new InternalServerErrorException(
        'An unexpected error occurred during Google authentication redirect.'
      );
    }
  }
  ...
  async requestPasswordReset(@Body() emailDto: EmailDto) {
    try {
      ...
      throw new InternalServerErrorException(
        'An unexpected error occurred during password reset code request.'
      );
    }
  }
  ...
  async selectCountry(
    ...
    return await this.authService.setCountry(
      userId,
      countryDto.country as CountryEnum,
      res
    );
  }
}
