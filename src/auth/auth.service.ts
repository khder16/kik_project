...
export class AuthService {
    ...
    async login(loginDto: LoginDto, res: Response) {
        ...
        try {
            ...
            this.logger.error(`Error during login process: ${error.message}`, error.stack);
        ...
        try {
            ...
            if (user) {
                if (provider === 'google' && !user.googleId) {
                    user = await this.userService.update(user._id.toString(), { googleId: profile.id });
                } else if (provider === 'facebook' && !user.facebookId) {
                    user = await this.userService.update(user._id.toString(), { facebookId: profile.id });
                }
            } else {
                const generatedPassword = await bcrypt.hash(Date.now().toString() + Math.random().toString(36).substring(2, 15), 10);
    ...
    async UpdatePassword(email: string, password: string) {
        try {
            const hashedPassword = await bcrypt.hash(password, 10);
            const result = await this.userService.updatePasswordUser(email, hashedPassword);
            return {
